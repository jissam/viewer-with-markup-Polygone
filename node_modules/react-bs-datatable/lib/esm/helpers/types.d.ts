 ...
 */
export declare type ColumnValueProcessor<TColumnType> = Partial<{
    [K in keyof TColumnType]: (columnValue: TColumnType[K]) => string | number;
}> | ((key: keyof TColumnType, rowValue: TColumnType) => string | number | TColumnType[keyof TColumnType]);
/**
 * This is the object of key/value which is used to transform a column's value
 * to another form.
 *
 ...
 */
export declare type ColumnProcessObj<TColumnType, TReturnType = string> = Partial<{
    [K in keyof TColumnType]: (columnValue: TColumnType[K]) => TReturnType;
}>;
/**
 * This is used for the `extend` keyword in the components.
 */
export declare type TableRowType<T = any> = Record<string, T>;
/**
 * The helper type for the filter change event.
 */
export declare type FilterOnChange = (nextState: string) => void;
/**
 * The helper type for the sort by next prop change event.
 */
export declare type SortOnChange = (nextProp: SortType) => void;
/**
 * The helper type for the sort by prop change event.
 */
export declare type SortByPropOnChange = (sortedProp: string) => void;
/**
 * The helper type for the sort by prop change event.
 */
export declare type PaginationOnChange = (nextState: number) => void;
/**
 * The helper type for the rows per page change event.
 */
export declare type RowsPerPageOnChange = (nextState: number) => void;
/**
 * The helper type for the checkbox change eveent.
 */
export declare type CheckboxOnChange = (params: {
    checkboxProp: string;
