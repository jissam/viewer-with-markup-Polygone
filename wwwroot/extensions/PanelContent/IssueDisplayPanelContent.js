...
import IssueUpdateForm from './IssueUpdateForm'; // Importez le composant du formulaire de modification
import { OBJECT_URN } from '../IssueDisplayPanel';

const IssueDisplayPanelContent = ({ onUpdate }) => {
  ...
  const handleSearchChange = (event) => {
    ...
    const filtered = issueData.filter(
      (row) =>
        row.issueTitle.toLowerCase().includes(event.target.value.toLowerCase()) ||
  ...
  const handleSort = (columnId) => {
    ...
    const sortedData = [...issueData].sort((a, b) => {
    ...
    onUpdate(issue.elementId);
    setSelectedRow(issue.issueId === selectedRow ? null : issue.issueId);
  };

  const getRowClassName = (issue) => {
    return issue.issueId === selectedRow ? "text-center whitespace-nowrap hover:bg-slate-100 bg-slate-100" : "text-center whitespace-nowrap hover:bg-slate-100";
  };
  const handleEditClick = (issue) => {
    setIsEditing(true); // Afficher le formulaire de modification
    setSelectedIssue(issue); // Passer les données de la ligne sélectionnée au formulaire
  };
  return (
    <div>
            ...
            <table className="divide-y divide-gray-300 border border-gray-300 " id="dataTable">
              <thead className="font-medium rounded-lg px-5 py-2.5">
                <tr>
                  ...
                  <th style={{ height: '5px', fontSize: '0.8rem' }} className="bg-slate-200 px-2 py-2 text-base text-left text-black border-2 border-gray-300 border-b-sky-600" onClick={() => handleSort('issueTitle')}>
                  ...
                  {/*<th className="bg-slate-200 px-2 py-2 text-sm text-left text-black border-2 border-gray-300 border-b-sky-600">Issue ID</th>*/}
              ...
              <tbody className="bg-white divide-y divide-gray-300">
                {filteredData.length > 0 ? (
                  filteredData
                    ...
                    .map((issue, index) => (
                      <tr key={index} className={getRowClassName(issue)} onClick={() => handleRowClick(issue)}>
                        <td className="px-2 py-2 text-xs text-left text-gray-700 border-2 border-gray-300 ">
                          <svg className="h-6 w-6 text-sky-600" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round" onClick={() => handleEditClick(issue)}>
                        ...
                        <td className="px-2 py-2 text-xs text-left text-gray-700">{issue.issueTitle}</td>
                        <td className="px-2 py-2 text-xs text-left text-gray-700">{issue.elementName}</td>
                        {/*<td className="px-2 py-2 text-xs text-left text-gray-700">{issue.issueId}</td>*/}
                        <td className="px-2 py-2 text-xs text-left text-gray-700">{issue.author}</td>
                        <td className="px-2 py-2 text-xs text-left text-gray-700">{issue.status}</td>
                        <td className="px-2 py-2 text-xs text-left text-gray-700">{issue.issues}</td>
                      </tr>
                    ))
                ) : (
                  <tr style={{ height: 53 }}>
          ...
          {isEditing && selectedIssue && (
            <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
              <div className="bg-white p-8 rounded-lg h-auto max-h-full max-w-xl">
                <IssueUpdateForm issue={selectedIssue} onClose={() => setIsEditing(false)} />
              </div>
            </div>
          )}
  ...
  />
      ...
      {filteredData && filteredData.length === 0 && (
        <p className="text-center mt-64">No Issues For This Model.</p>
      )}
    </div>
  );
...
export default IssueDisplayPanelContent;
